...
import { newSessionData, session } from "../session";

declare const DragonCopilotSDK: any;

// Bind the runtime global from the CDN and cast to the typed namespace
export const dragon = (globalThis as any).DragonCopilotSDK
  ?.dragon as typeof Dragon;

export type RecordingMode = "ambient" | "dictation" | null;
export type AmbientUploadStatus =
  | "uploading"
  | "uploadCompleted"
  | "uploadFailed";
/**
...
export class DragonService {
  ...
  public onRecordingModeChanged$: Observable<RecordingMode> =
    this.recordingMode$.asObservable();
  ...
  public onProcessingDictationChanged$: Observable<boolean> =
    this.processingDictation$.asObservable();

  private hasMicrophone$ = new BehaviorSubject<boolean>(false);
  public onHasMicrophoneChanged$: Observable<boolean> =
    this.hasMicrophone$.asObservable();

  private initialized$ = new BehaviorSubject<boolean>(false);
  public onInitializedChanged$: Observable<boolean> =
    this.initialized$.asObservable();

  private ambientUploadStatus$ =
    new BehaviorSubject<AmbientUploadStatus | null>(null);
  ...
  /** Retrieves the URLs for the Dragon Copilot SDK setting views. */
  async getUrls(): Promise<dragon.settingViews.SettingViewsUrls | null> {
    const urls = await dragon.settingViews.getUrls();
    return urls;
  }

  /** Navigates to the specified settings view within the Dragon Copilot SDK iframe. */
  navigateToSettingView(view: string): void {
    dragon.settingViews.navigateToSettingView(view);
  }

  /** Sets the ambient session data for the Dragon Copilot SDK. */
  setSessionData(): Promise<void> {
    return dragon.recording.ambient.setSessionData(newSessionData());
  }
  ...
  #attachEventHandlers() {
    ...
    dragon.recording.events.addEventListener(
    ...
    dragon.microphone.events.addEventListener(
      "microphoneListChanged",
      (event) => {
    ...
    dragon.recording.ambient.events.addEventListener(
      ...
      (event) => {
        this.ambientUploadStatus$.next(
          event.detail.status as AmbientUploadStatus,
        );
      },
    );
  }
...
export function getDragonService(authService?: AuthService): DragonService {
  if (!dragonServiceInstance) {
    if (!authService) {
      throw new Error(
        "AuthService is required to create DragonService instance",
      );
    }
